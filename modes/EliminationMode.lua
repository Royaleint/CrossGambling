-- Elimination: everyone rolls each round and the lowest roller is out. When two remain, it
-- switches to DeathRoll rules to decide the winner, and every eliminated player pays them.
-- Round state lives in game.elimination:
--   alive            set of names still in
--   pending          set of names expected to roll this round
--   eliminationOrder list of { name, round } in the order they went out
--   round            current round number
--   finale           true once it is down to two
--   finaleOrder      the two finalists, finaleTurnIndex whose turn, finaleMax current ceiling
...
-- A player was removed (banned) mid-round or mid-finale. elim.alive/elim.pending are updated here
-- because nothing else does - without this, a stale name lingers in elim.alive and the round/finale
-- math built on it (CountKeys, the two finalists) goes wrong.
function EliminationMode:OnPlayerLeave(addon, game, playerName)
    ...
    -- The leaver was the only one still owed a roll this round - nobody else will trigger the
    -- round's resolution, so do it now instead of leaving the game stuck waiting on them.
